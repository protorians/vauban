...
export class ServerActons {
    static manifestSlug = "server-actions.manifest.json";
    static actions = new Map();
    static get manifestFile() {
        return path.join(Vauban.workDir, this.manifestSlug);
    }
    static manifest() {
        return Vauban.workDir.trim().length > 1
            ? JSON.parse((FileManager.safeRead(this.manifestFile, 'utf-8', '{}'))
                .toString()) : {};
    }
    static update(entries) {
        try {
    ...
    static async getActions() {
        for (const [serial, pathname] of Object.entries(this.manifest())) {
            ...
            const file = path.join(Vauban.appDir, pathname);
            const mod = await HMR.import(file);
            const fn = mod[name] || mod.default || undefined;
            if (fn)
                this.actions.set(serial, fn);
        }
        return this.actions;
    }
    static async gateway(socket) {
